use super::{block::Block, func::Func, inst::{BinaryOpcode, InstData}, types::Type, value::Value};
...
impl <'f> Builder<'f> {
    ...
    fn emit_binary_inst(&mut self, opcode: BinaryOpcode, res_type: Type, args: [Value; 2]) -> Value {
        let inst = self.func.add_inst(self.insert_point, InstData::Binary{opcode, args}, res_type);
    ...
    pub fn emit_add(&mut self, lhs: Value, rhs: Value) -> Value {
        self.emit_binary_inst_same_types(BinaryOpcode::Add, [lhs, rhs])
    }

    pub fn emit_uadd(&mut self, lhs: Value, rhs: Value) -> Value {
